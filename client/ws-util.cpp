...
bool ShutdownConnection(SOCKET sd) // sd = socket descriptor
{
	...
	while (true) {
		...
		if (nNewBytes == SOCKET_ERROR) {
			...
			if (sock_error == WSAEWOULDBLOCK) {  // no data available - error 10035 and normal for non-blocking sockets - ignores sending any error messages
				// printf("WSAEWOULDBLOCK error"); // for heaving debugging				
				break;
			}
			// I think <WSAECONNRESET> could be a little smoother but, it may not matter - changed for smoother shutdown - in words
			else if (sock_error == WSAECONNABORTED || sock_error == WSAECONNRESET) { // aborted server connection, results in closed socket
		...
		else { // Okay, we're done! nNewBytes == 0 = nothing
