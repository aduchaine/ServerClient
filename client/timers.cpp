...
bool Timers::CheckTimer(int timer_time)
{
	...
	if (restart == true && current_time - start_time > timer_time) {
		start_time = current_time;
		end_time = start_time + timer_time;
		//std::cout << "\n Check Timer: restart(true) - current:(" << current_time << ") start:(" << start_time << ") timer:(" << timer_time << ") end:(" << end_time << ")" << std::endl; // heavy debugging
		return true;
	}
	else if (restart == false && current_time - start_time > timer_time) {		
		//std::cout << "\n Check Timer: restart(false) - current:(" << current_time << ") start:(" << start_time << ") timer:(" << timer_time << ") end:(" << end_time << ")" << std::endl; // heavy debugging
		enabled = false;
		return true;
	}
	else {
